...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-border/50 last:border-0">
      <span className="text-[11px] font-black uppercase tracking-wider text-muted-foreground shrink-0">
        {label}
      </span>
...
function Section({
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="bg-muted/30 rounded-xl px-4 py-1">{children}</div>
    </div>
  );
}
          ...
          <div className="p-6 space-y-8 flex-1">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <Section title="Identification" icon={QrCode}>
                <InfoRow
                  label="UHID"
                  value={
                    (detail.patient as any)?.uhid ||
                    detail.patientId.substring(0, 8)
                  }
                />
            ...
            <Section title="Tests in Report" icon={Beaker}>
              <div className="py-2 flex flex-wrap gap-2 items-center">
                {(
                  (detail as any)?.diagnosticTests ||
                  (detail as any)?.testItems ||
                  []
                ).length > 0 ? (
                  (
                    (detail as any)?.diagnosticTests ||
                    ...
                    <Badge
                      key={i}
                      variant="secondary"
                      className="bg-blue-500/5 text-blue-700 font-bold border-blue-100 px-3 py-1"
                    >
                      ...
                      {result.blocks.map((block) => {
                        if (block.type === "header") {
                          return (
                            <div
                              key={block.id}
                              className="py-2 border-b-2 border-primary/10"
                            >
                          ...
                          return (
                            <div
                              ...
                              className={cn(
                                ...
                                block.isHeader &&
                                  "bg-primary/5 px-2 rounded-lg py-2 my-1",
                                block.isAbnormal &&
                                  "bg-red-500/5 px-2 rounded-lg",
                              )}
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <div className="mt-1 text-[9px] text-muted-foreground/60 font-medium italic leading-tight">
                                  ...
                                  <span
                                    dangerouslySetInnerHTML={{
                                      __html: block.referenceRange.replace(
                                        /\n/g,
                                        "",
                                      ),
                                    }}
                        ...
                        if (block.type === "narrative") {
                          return (
                            <div
                              key={block.id}
                              className="p-3 bg-card/50 rounded-xl border border-border/40 my-2"
                            >
                              <div
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <p
                              className={cn(
                                "text-xs font-semibold truncate",
                                row.isHeader &&
                                  "font-black text-blue-800 uppercase tracking-tight",
                                row.isBold &&
                            ...
                            {!row.isHeader && row.referenceRange && (
                              <div className="text-[9px] text-muted-foreground mt-0.5 leading-tight">
                                ...
                                <span
                                  dangerouslySetInnerHTML={{
                                    __html: row.referenceRange.replace(
                                      /\n/g,
                                      "",
                                    ),
                                  }}
                          ...
                          {!row.isHeader && (
                            <div className="flex items-center gap-3 shrink-0">
                              ...
                              {row.isAbnormal && (
                                <Badge className="bg-red-600/10 text-red-600 border-red-600/20 text-[9px] font-black italic px-1 h-5 rounded hover:bg-red-600/10">
                                  HB
                                </Badge>
                              )}
                            </div>
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      {result.interpretation && (
                        <div className="pt-3 border-t border-dashed">
                          <p className="text-xs font-black uppercase text-muted-foreground mb-1 underline">
                            Impression / Comment
                          </p>
            ...
            <div className="grid grid-cols-2 gap-6">
              <Section title="Collection" icon={Beaker}>
                <InfoRow
                  label="Status"
                  value={detail.isSampleCollected ? "Collected" : "Pending"}
                />
                <InfoRow
                  label="At"
                  value={format(new Date(detail.updatedAt), "dd MMM, hh:mm a")}
                />
              </Section>
              <Section title="Timings" icon={Clock}>
                <InfoRow
                  label="Requested"
                  value={format(new Date(detail.createdAt), "dd MMM, hh:mm a")}
                />
                <InfoRow
                  label="Updated"
                  value={format(new Date(detail.updatedAt), "dd MMM, hh:mm a")}
            ...
            <div className="space-y-3 bg-indigo-500/5 p-5 rounded-2xl border border-indigo-500/10">
              ...
              <p className="text-[10px] text-muted-foreground font-medium italic">
                Handover status tracking for patient relations and billing
                reconciliation.
              </p>
            </div>
