...
export function AmbulancePrintDialog({ open, onOpenChange, booking }: AmbulancePrintDialogProps) {
    ...
    const netTotal = Number(booking.netFare || 0)
    const paidAmount = Number(booking.paidAmount || 0)
    const dueAmount = Number(booking.dueAmount || 0)
    const discountAmount = Number(booking.discountAmount || 0)
    const isFullyPaid = dueAmount <= 0
    const invoiceNumber = booking.id ? booking.id.substring(0, 8).toUpperCase() : "N/A" // Use short ID if no explicit invoice number exists
            ...
            <div className="border border-black mb-2 relative z-10 mt-2 text-[13px]">
                <div className="grid grid-cols-2 border-b border-black">
                    <div className="p-1 px-3 border-r border-black font-bold flex items-center">
                        Patient ID : {booking.patient?.patientNumber || 'N/A'}
                    </div>
                ...
                <div className="grid grid-cols-2 border-b border-black">
                    <div className="p-1 px-3 border-r border-black font-bold flex items-center">
                        Name <span className="mx-2">:</span> {booking.patientName || 'N/A'}
                    </div>
                    <div className="p-1 px-3 font-bold flex items-center">
                        <span className="w-12">Date</span> <span className="mr-2">:</span> {new Date(booking.createdAt).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }).toUpperCase()}
                    </div>
                </div>
                <div className="grid grid-cols-1 border-b border-black">
                    <div className="p-1 px-3 font-bold flex flex-wrap items-center">
                        <div className="flex gap-3 mr-8">
                            <span>Contact : {booking.guardianPhone || booking.patient?.phone || 'N/A'}</span>
                        </div>
                    </div>
                </div>
                <div className="grid grid-cols-1 border-b border-black">
                    <div className="p-1 px-3 font-bold flex items-start">
                        <span className="shrink-0 w-24">Vehicle :</span>
                        <span className="uppercase text-[12px] pt-0.5">{booking.ambulance?.vehicleNumber || "Pending"}</span>
                    </div>
                </div>
                <div className="p-1 px-3 font-bold flex items-center border-b border-black">
                    <span className="w-24 uppercase">Driver:</span>
                    <span>{booking.ambulance?.driverName || "N/A"}</span>
                </div>
                <div className="grid grid-cols-2">
                    <div className="p-1 px-3 font-bold flex items-center border-r border-black">
                        <span className="w-16 uppercase text-[10px]">Pickup:</span>
                        <span className="text-[11px] leading-tight">{booking.pickupLocation}</span>
                    </div>
                    <div className="p-1 px-3 font-bold flex items-center">
                        <span className="w-16 uppercase text-[10px]">Dropoff:</span>
                        <span className="text-[11px] leading-tight">{booking.dropoffLocation}</span>
                    </div>
                </div>
            </div>
    
            {/* Financial Settlement */}
            <div className="flex justify-between items-start mt-4 mb-4 relative z-10 text-[13px]">
                ...
                <div className="w-[250px]">
                    <div className="flex justify-between py-0.5 font-black">
                        ...
                        <span>{Number(booking.totalFare || 0).toFixed(2)} ৳</span>
                    </div>
                    {discountAmount > 0 && (
    ...
    const handlePrint = () => {
        const printContent = document.getElementById('ambulance-receipt-content')?.innerHTML;
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[800px] p-0 overflow-hidden bg-white border-none shadow-2xl light max-h-[95vh]">
                ...
                <div className="p-0 max-h-[85vh] overflow-y-auto print:max-h-none print:p-0 bg-white text-black" id="ambulance-receipt-content" style={{ width: "100%", maxWidth: "210mm", margin: "0 auto" }}>
                    <>
                        <ReceiptContent copyTitle="AMBULANCE BOOKING — OFFICE COPY" />
                        <div className="page-break" />
                        <ReceiptContent copyTitle="AMBULANCE BOOKING — CUSTOMER COPY" />
                    </>
                </div>
            </DialogContent>
        </Dialog>
