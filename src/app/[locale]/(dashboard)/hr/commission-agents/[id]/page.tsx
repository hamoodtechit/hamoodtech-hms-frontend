                ...
                <div className="md:col-span-8 space-y-6">
                    <div className="grid gap-4 sm:grid-cols-3">
                        <Card className="bg-primary/5 border-primary/10">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">{yearlyStats?.totalSalesCount || 0}</div>
                        ...
                        <Card className="bg-emerald-500/5 border-emerald-500/10">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">{formatCurrency(yearlyStats?.totalSalesAmount || 0)}</div>
                        ...
                        <Card className="bg-amber-500/5 border-amber-500/10">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-amber-600">{formatCurrency(yearlyStats?.totalCommissionEarned || 0)}</div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="flex flex-col items-center justify-center p-8 space-y-2 border-2 border-dashed rounded-xl border-zinc-100 dark:border-white/5">
                                ...
                                <h3 className="text-lg font-bold">Year {yearlyStats?.year || new Date().getFullYear()} Statistics</h3>
                                <div className="grid grid-cols-2 gap-x-12 gap-y-4 w-full max-w-md pt-4">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-lg font-semibold">{yearlyStats?.commissionPercentage || agent.commissionPercentage}%</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-xs text-muted-foreground uppercase font-bold">Sales Count</p>
                                        <p className="text-lg font-semibold">{yearlyStats?.totalSalesCount || 0}</p>
                                    </div>
                                    <div className="space-y-1 border-t pt-2">
                                        <p className="text-xs text-muted-foreground uppercase font-bold">Total Sales</p>
                                        <p className="text-lg font-semibold">{formatCurrency(yearlyStats?.totalSalesAmount || 0)}</p>
                                    </div>
                                    <div className="space-y-1 border-t pt-2">
                                        <p className="text-xs text-emerald-600 uppercase font-bold">Total Commission</p>
                                        <p className="text-lg font-bold text-emerald-600">{formatCurrency(yearlyStats?.totalCommissionEarned || 0)}</p>
                                    </div>
                                </div>
                            </div>
                        </CardContent>
